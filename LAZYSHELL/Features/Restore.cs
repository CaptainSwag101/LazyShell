...
namespace LAZYSHELL
{
    public partial class Restore : Form
    {
        ...
        private void buttonOK_Click(object sender, EventArgs e)
        {
            ...
            {
                Buffer.BlockCopy(data, 0x042748, Model.Data, 0x042748, 73 * 3);    // spc track pointers
                Buffer.BlockCopy(data, 0x045526, Model.Data, 0x045526, 0x1B413);    // spc track data
            }
            if (elements.Nodes["Audio"].Nodes["EventSoundFX"].Checked)
            {
                Buffer.BlockCopy(data, 0x042826, Model.Data, 0x042826, 0x1600);    // event sound fx
            }
            if (elements.Nodes["Audio"].Nodes["BattleSoundFX"].Checked)
            {
                Buffer.BlockCopy(data, 0x043E26, Model.Data, 0x043E26, 0x1600);    // battle sound fx
            }
            // Battlefields
            ...
            {
                Buffer.BlockCopy(data, 0x3E0E68, Model.Data, 0x3E0E68, 0x3E2C80 - 0x3E0E68);    // compressed data
                Buffer.BlockCopy(data, 0x3E99E0, Model.Data, 0x3E99E0, 0x3EB94A - 0x3E99E0);    // compressed data
                Buffer.BlockCopy(data, 0x3EEF00, Model.Data, 0x3EEF00, 0x3EF700 - 0x3EEF00);    // compressed data
            }
            // Mini-games
            if (elements.Nodes["MiniGames"].Nodes["MineCarts"].Checked)
            {
                Buffer.BlockCopy(data, 0x388000, Model.Data, 0x388000, 0x8000);
            }
            // Monsters
            ...
            {
                Buffer.BlockCopy(data, 0x3EF800, Model.Data, 0x3EF800, 0x30); // world map data
            }
            if (elements.Nodes["WorldMaps"].Nodes["Locations"].Checked)   // locations
            {
                Buffer.BlockCopy(data, 0x3EF830, Model.Data, 0x3EF830, 0x380);  // location data
                Buffer.BlockCopy(data, 0x3EFD00, Model.Data, 0x3EFD00, 0x220);  // location names
            }
            if (elements.Nodes["WorldMaps"].Nodes["Tilesets"].Checked)   // world map tilesets
            {
                Buffer.BlockCopy(data, 0x3E0000, Model.Data, 0x3E0000, 0x0E68); // world map logo, banners
                Buffer.BlockCopy(data, 0x3E2E82, Model.Data, 0x3E2E82, 0x69B6); // world map graphics, tilesets
            }
            this.Close();
        }

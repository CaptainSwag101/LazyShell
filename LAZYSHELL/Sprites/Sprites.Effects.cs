...
namespace LAZYSHELL
{
    public partial class Sprites
    {
        ...
        private void InitializeE_Frames()
        {
            ...
            if (e_animations[e_currentAnimation].Frames.Count == 0)
            {
                ...
                if (e_animations[e_currentAnimation].FramePacketPointer == 0xFFFF)
                {
                    ...
                    MessageBox.Show("This is a dummied sequence. It cannot contain any frames.",
        ...
        private void e_insertFrame_Click(object sender, EventArgs e)
        {
            ...
            {
                MessageBox.Show(
                    "Could not insert a new frame.\n\nSequences cannot contain more than 64 frames total.", "LAZY SHELL",
            ...
            if (e_availableBytesNum - 2 < 0)
            {
                int bank = effects[currentEffect].AnimationPacket < 39 ? 0x330000 : 0x340000;
                MessageBox.Show(
                    "Could not insert a new frame.\n\nNot enough available bytes in bank 0x" + bank.ToString("X6") + ".", "LAZY SHELL",
                    MessageBoxButtons.OK, MessageBoxIcon.Information);
                return;
            }
        ...
        private void e_frameMold_ValueChanged(object sender, EventArgs e)
        {
            ...
            if (e_frameMold.Value >= e_animations[e_currentAnimation].Molds.Count)
            {
                ...
                MessageBox.Show("Mold for frame #" + e_currentFrame.ToString() + " is not valid. Change to lower value.", "LAZY SHELL");
                return;
            }
        ...
        private void e_molds_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            //{
            //    MessageBox.Show("Mold is invalid. Fix this later.", "LAZY SHELL");
        ...
        private void e_insertMold_Click(object sender, EventArgs e)
        {
            ...
            {
                MessageBox.Show(
                    "Could not insert a new mold.\n\nEffects cannot contain more than 32 molds total.", "LAZY SHELL",
                    MessageBoxButtons.OK, MessageBoxIcon.Information);
                return;
            }
            if (e_availableBytesNum - 2 < 0)
        ...
        private void moldInsertTile_Click(object sender, EventArgs e)
        {
            ...
            {
                MessageBox.Show(
                    "Could not insert a new tile.\n\nEffects cannot contain more than 256 tiles total.", "LAZY SHELL",
                    MessageBoxButtons.OK, MessageBoxIcon.Information);
                return;
            }
                ...
                if (colEditColors.GetItemChecked(index))
                {
                    switch (coleditSelectCommand.SelectedIndex)
                    {
                        ...
                        case 6:
                            ...
                            break;
                        case 7:
                            if (reds[i] == greens[i] && reds[i] == blues[i]) break;
                            double grey = (reds[i] * 0.3) + (greens[i] * 0.59) + (blues[i] * 0.11);
                            reds[i] = (int)Math.Round(grey, MidpointRounding.ToEven);
                            greens[i] = (int)Math.Round(grey, MidpointRounding.ToEven);
                            blues[i] = (int)Math.Round(grey, MidpointRounding.ToEven);
                            break;
                    }
                }
            }

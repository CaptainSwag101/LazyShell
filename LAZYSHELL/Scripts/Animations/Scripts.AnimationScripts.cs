...
namespace LAZYSHELL.ScriptsEditor
{
    public partial class Scripts : Form
    {
        ...
        private LAZYSHELL.Previewer.BattleDialoguePreview battleDialoguePreview;
        private LAZYSHELL.Previewer.MenuTextPreview menuTextPreview;
        ...
        private void InitializeAnimationScriptsEditor()
        {
            ...
            this.fontCharacters = scriptsModel.FontCharacters;
            this.menuTextPreview = new LAZYSHELL.Previewer.MenuTextPreview();
            this.battleDialoguePreview = new LAZYSHELL.Previewer.BattleDialoguePreview();
            ...
            switch (asc.Opcode)
            {
                case 0x00:
                    ...
                    for (int i = 0; i < settings.SpriteNames.Count; i++)
                        aniNameA.Items.Add("[" + i.ToString("d4") + "]  " + settings.SpriteNames[i]);
        ...
        private void animationName_DrawItem(object sender, DrawItemEventArgs e)
        {
                ...
                animationCategory.SelectedIndex == 6)
            {
                ...
                {
                    case 1: 
                        arr = universal.SpellNames.GetNameByNum(e.Index + 0x40).ToCharArray();
                        temp = battleDialoguePreview.GetPreview(fontCharacters, palette, arr, false); break;
                    case 2: 
                        arr = universal.AttackNames.GetName(e.Index).ToCharArray();
                        temp = battleDialoguePreview.GetPreview(fontCharacters, palette, arr, false); break;
                    case 4: 
                        arr = universal.ItemNames.GetNameByNum(e.Index + 0x60).ToCharArray();
                        temp = menuTextPreview.GetPreview(menuCharacters, palette, arr, false); break;
                    case 5:
                        arr = universal.SpellNames.GetNameByNum(e.Index).ToCharArray(); 
                        temp = menuTextPreview.GetPreview(menuCharacters, palette, arr, false); break;
                    case 6:
                        arr = universal.ItemNames.GetNameByNum(e.Index).ToCharArray();
                        temp = menuTextPreview.GetPreview(menuCharacters, palette, arr, false); break;
                    default: 
                        arr = new char[1];
                        temp = menuTextPreview.GetPreview(menuCharacters, palette, arr, false); break;
        ...
        private void animationScriptTree_AfterSelect(object sender, TreeViewEventArgs e)
        {
            ...
            for (int i = 0; i < asc.AnimationData.Length; i++)
            {
                ...
                numUpDown.MouseMove += new MouseEventHandler(controlMouseMove);
                numUpDown.Tag = asc;

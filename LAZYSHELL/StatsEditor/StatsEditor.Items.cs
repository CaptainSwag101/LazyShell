...
namespace LAZYSHELL.StatsEditor
{
    public partial class StatsEditor
    {
        ...
        private void itemName_DrawItem(object sender, DrawItemEventArgs e)
        {
            ...
            // set the pixels
            int[] temp = menuTextPreview.GetPreview(menuCharacters, palette, statsModel.Items[universal.ItemNames.GetNumFromIndex(e.Index)].Name, false);
        ...
        private void textBoxItemName_TextChanged(object sender, EventArgs e)
        {
            ...
            if (statsModel.Items[(int)itemNum.Value].Name != raw)
            {
                statsModel.Items[(int)itemNum.Value].Name = raw;

                universal.ItemNames.SwapName(
                    statsModel.Items[(int)itemNum.Value].ItemNum,
                    new string(statsModel.Items[(int)itemNum.Value].Name));
                universal.ItemNames.SortAlpha();
        ...
        {
            statsModel.Items[(int)itemNum.Value].Name[0] = (char)(itemNameIcon.SelectedIndex + 0x20);
            itemName.Invalidate();
        }

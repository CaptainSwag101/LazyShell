    ...
    public class Formation
    {
        ...
        public string[] FormationName
        {
            get
            {
                ...
                {
                    if (formationUse[i])
                        names[i] = RawToASCII(monsters[formationMonster[i]].Name, Settings.Default.KeystrokesMenu);
                    else
                        names[i] = "{unused}";
                }
                return names;
            ...
            get
            {
                ...
                for (int i = 0; i < 8; i++)
                {
                    if (formationUse[i] == true)
                        formationNameList +=
                            RawToASCII(monsters[formationMonster[i]].Name, Settings.Default.KeystrokesMenu) +
                            ((i < 7) ? "  /  " : "");
                    else

...
namespace LAZYSHELL.StatsEditor
{
    public partial class StatsEditor
    {
        ...
        private void spellName_DrawItem(object sender, DrawItemEventArgs e)
        {
            ...
            int[] pixels;
            Bitmap icon;
            if (universal.SpellNames.GetNumFromIndex(e.Index) < 64)
            {
                temp = menuTextPreview.GetPreview(menuCharacters, palette, statsModel.Spells[universal.SpellNames.GetNumFromIndex(e.Index)].Name, false);
            ...
            {
                temp = battleDialoguePreview.GetPreview(fontCharacters, palette, statsModel.Spells[universal.SpellNames.GetNumFromIndex(e.Index)].Name, false);
        ...
        {
            statsModel.Spells[(int)spellNum.Value].Name[0] = (char)(spellNameIcon.SelectedIndex + 0x20);
            spellName.Invalidate();
        }
        private void spellNameIcon_DrawItem(object sender, DrawItemEventArgs e)
        {
            ...
            int[] temp;
            if (spellNum.Value < 64)
                temp = menuTextPreview.GetPreview(menuCharacters, palette, test, false);
            else
                temp = menuTextPreview.GetPreview(fontCharacters, palette, test, false);
        ...
        private void textBoxSpellName_TextChanged(object sender, EventArgs e)
        {
            ...
            {
                statsModel.Spells[(int)spellNum.Value].Name = raw;

                universal.SpellNames.SwapName(
                    statsModel.Spells[(int)spellNum.Value].SpellNum,
                    new string(statsModel.Spells[(int)spellNum.Value].Name));
                universal.SpellNames.SortAlpha();
                this.spellName.Items.Clear();

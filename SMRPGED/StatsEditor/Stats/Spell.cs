...
namespace SMRPGED.StatsEditor.Stats
{
    public class Spell
    {
        ...
        public string Name { get { return this.name; } set { this.name = PadString(value, 15); } }
        ...
        public string GetDescription(bool symbols) { if (!descriptionError) return new string(textHelperReduced.DecodeText(description, symbols)); else return new string(description); }
        ...
        public ushort Assemble(ushort descOffset)
        {
            ...
            BitManager.SetByteArray(data, 0x3A137F + (spellNum * 15), strToByte(name));
        ...
        {
            int namePtr = 0x3A137F;
            byte[] temp = new byte[15];

            for (int i = 0; i <= 14; i++)
            {
                temp[i] = (byte)data[namePtr + (spellNum * 15) + i];

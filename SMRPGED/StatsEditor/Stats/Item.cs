...
namespace SMRPGED.StatsEditor.Stats
{
    public class Item
    {
        ...
        public string Name
        {
            ...
            {
                if (this.icon == 0)
                    this.name = PadString(value, 15);
                else
                    this.name = PadString(value, 14);
            }
        }
        ...
        {
            if (!descriptionError) return new string(textHelperReduced.DecodeText(description, symbols));
        ...
        public byte Icon { get { return this.icon; } set { this.icon = value; } }
        ...
        private void InitializeItem()
        {
            ...
            int offset = (itemNum * 15) + 0x3A46EF;
            icon = data[offset];
        ...
        {
            ushort retLength = 0;
            byte[] temp = strToByte(name);
            int offset = 0x3A46EF + (itemNum * 15);
            data[offset] = icon;
            BitManager.SetByteArray(data, offset + 1, strToByte(name));
        ...
        {
            int namePtr = 0x3A46EF;
            int h = 14;
            bool iconTrue = false;
            byte[] temp = new byte[15];
            byte tempB = BitManager.GetByte(data, namePtr + (itemNum * 15));

            if (tempB >= 0x30 && tempB <= 0x39) // Numbers
                iconTrue = false;
            else if (tempB >= 0x41 && tempB <= 0x5A) // Lower Case
                iconTrue = false;
            else if (tempB >= 0x61 && tempB <= 0x7A) // Upper Case
                iconTrue = false;
            else // Icon
                iconTrue = true;

            iconTrue = true;    // probably better to just have it true always (2009-06-02)
            if (iconTrue == true)
            {
                namePtr++;
                h = 13;
            ...
            for (int i = 0; i <= h; i++)
            {
                temp[i] = (byte)data[namePtr + (itemNum * 15) + i];

...
namespace SMRPGED.ScriptsEditor
{
    ...
    {
        private int eventNum; public int EventNum { get { return this.eventNum; } set { this.eventNum = value; } }
        ...
        public EventScript(byte[] data, int eventNum)
        {
            this.data = data;
            this.eventNum = eventNum;

            InitializeEventScript(eventNum);
        }
        public EventScript(byte[] data, byte[] eventData, int eventNum)
        {
            this.data = data;
            this.eventNum = eventNum;
        ...
        private void InitializeEventScript(int eventNum)
        {
            ...
            {
                bank = 0x1E0000;
                eventNumTemp = eventNum;
            }
            else if (eventNum >= 1536 && eventNum <= 3071)
            {
                bank = 0x1F0000;
                eventNumTemp = eventNum - 1536;
            }
            else if (eventNum >= 3072 && eventNum <= 4095)
            {
                bank = 0x200000;
                eventNumTemp = eventNum - 3072;
            }
            else
            ...
            offset = BitManager.GetShort(data, bank + eventNumTemp * 2);
        ...
        private void ParseEventScript(byte[] es)
        {
            ...
            while (offset < es.Length)
            {
                switch (eventNum)
                {
                    ...
                    case 0xE91: if (eventNum == 0xE91) { if (offset != 0x3C4) goto default; len = 0x51; }
                        temp = new EventScriptCommand(BitManager.GetByteArray(es, offset, len), this.baseOffset + offset);
                        temp.EmbeddedActionQueue = new ActionQueue(BitManager.GetByteArray(temp.EventData, 0, temp.EventData.Length), -1, temp.Offset);
                        temp.IsDummy = true;
                        commands.Add(temp);
                        break;
                    default:
        ...
        public int GetEventOpcodeLength(byte[] es, int offset)
        {
            ...
            {
                if (BitManager.GetBit(es[offset + 2], 7))

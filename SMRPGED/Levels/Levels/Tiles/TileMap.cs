    ...
    class TileMap
    {
        ...
        private void ChangeSingleTile(int layer, int placement, int tile, int x, int y, bool pasting)
        {
            ...
            if (!source.GetPriority1(0)) // tile 0
                CopyOverTile8x8(source.GetSubtile(0), layerA, 1024, x, y);
            else
                CopyOverTile8x8(source.GetSubtile(0), layerB, 1024, x, y);
            if (!source.GetPriority1(1)) // tile 1
                CopyOverTile8x8(source.GetSubtile(1), layerA, 1024, x + 8, y);
            else
                CopyOverTile8x8(source.GetSubtile(1), layerB, 1024, x + 8, y);
            if (!source.GetPriority1(2)) // tile 2
                CopyOverTile8x8(source.GetSubtile(2), layerA, 1024, x, y + 8);
            else
                CopyOverTile8x8(source.GetSubtile(2), layerB, 1024, x, y + 8);
            if (!source.GetPriority1(3)) // tile 3
                CopyOverTile8x8(source.GetSubtile(3), layerA, 1024, x + 8, y + 8);
            else
                CopyOverTile8x8(source.GetSubtile(3), layerB, 1024, x + 8, y + 8);
        ...
        private void CreateLayer(int layer)
        {
            ...
            if (layer != 2) // Layers 1 and 2
            {
                ...
                {
                    tileNum = BitManager.GetShort(tileMaps[layer], offset);
                    if (tileNum > 0x1FF)
                    {
            ...
            else // Layer 3
            {
                for (int i = 0; i < tileMaps[layer].Length / increment; i++)
                {
                    tileNum = BitManager.GetByte(tileMaps[layer], offset);
                    if (tileNum > 0xFF)
                    {
        ...
        private int[] DrawLayerByPriorityOne(int[] dest, int layer, bool priority)
        {
            ...
            for (int i = 0; i < layers[layer].Length; i++)
            {
                for (int z = 0; z < 4; z++)
                {
                    if (layers[layer][i].GetPriority1(z) == priority)
                    {
                        switch (z)
                        {
                            case 0:
                                CopyOverTile8x8(layers[layer][i].GetSubtile(z), dest, 1024, (i % 64) * 16, (i / 64) * 16);
                                break;
                            case 1:
                                CopyOverTile8x8(layers[layer][i].GetSubtile(z), dest, 1024, (i % 64) * 16 + 8, (i / 64) * 16);
                                break;
                            case 2:
                                CopyOverTile8x8(layers[layer][i].GetSubtile(z), dest, 1024, (i % 64) * 16, (i / 64) * 16 + 8);
                                break;
                            case 3:
                                CopyOverTile8x8(layers[layer][i].GetSubtile(z), dest, 1024, (i % 64) * 16 + 8, (i / 64) * 16 + 8);
                                break;
                            default:
                                break;
                        }
        ...
        */
        ...
        public void AssembleIntoModel()
        {
            for (int z = 0; z < 2; z++)
            {
                ...
                {
                    BitManager.SetShort(tileMaps[z], i * 2, (ushort)layers[z][i].TileNumber);
                }
            ...
            if (layers[2] != null)
            {
                ...
                {
                    BitManager.SetByte(tileMaps[2], i, (byte)layers[2][i].TileNumber);
                }
            }
        }
    }

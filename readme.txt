...
A: These are unknown bits. "B0,b0" means "Byte 0, bit 0" and refers to
   ...
   feel free to post it in the bug report thread:
   http://acmlm.kafuka.org/board/thread.php?id=6770
...
A: Make sure the "Run dlg: whatever" command has the dialogue property
   "asynchronous" unchecked so any following commands that would change
   a sprite's animation sequence or whatever will run while the
   dialogue is playing. Use the "Pause script, resume on next dlg page"
   commands to pause the script after the animation is complete.
